...
## لوحة Web Dashboard اللحظية

يخدم `internal-server/public` لوحة عربية RTL على `/` محمية بجلسة Admin مستقلة. بعد تسجيل الدخول تقرأ الصفحة Snapshot من `/admin/data` وتتصل بـ `/ws`. يبث السيرفر Snapshot كاملاً بعد كل مزامنة وتعديل موظف أو معاملة أو إجازة أو مهمة أو حضور أو مستند أو مرفق. يعيد المتصفح الاتصال تلقائياً، ثم يجلب Snapshot جديداً قبل متابعة الأحداث.

يرسل EXE إلى `/api/heartbeat` كل 15 ثانية حالة الموقع، وعدد العمليات المعلقة، وآخر مزامنة. تعتبر اللوحة EXE غير متصل بعد 45 ثانية بلا heartbeat. كلمات المرور وtokens ومفاتيح API تُزال من بيانات Dashboard ولا تُرسل عبر WebSocket.
